...
def check_tautologies(tool_name, tool_input)
  ...
  matches = TAUTOLOGY_PATTERNS.grep(new_string)
...
def track_edit(tool_name, tool_input, _tool_response)
...
def normalize_error(response_str)
  ...
  ERROR_SIGNATURES.each do |signature, patterns|
    return signature if patterns.any? { |p| response_str.match?(p) }
  end
...
def track_error_signature(signature, _tool_name, response_str)
...
def detect_actual_failure(tool_name, tool_response)
  ...
  if tool_name == 'Bash'
    ...
    return nil if stdout.match?(/no such file|not found/i) && !stdout.match?(/^(bash|sh|ruby|python|node):\s/i)
  end
