...
module SanePromptIntelligence
  ...
  def check_past_learnings
    ...
    return "PATTERN: #{correction_count} corrections in recent prompts. Read user message carefully." if correction_count >= 3
  ...
  def format_patterns_for_user(patterns)
    ...
    lines << 'Trend: declining' if patterns[:score_trend] == :declining
