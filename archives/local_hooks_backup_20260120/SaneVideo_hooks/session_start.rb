...
def reset_mcp_verification
  ...
  StateManager.update(:mcp_health) do |health|
    ...
    health[:mcps].each_value do |data|
...
def show_mcp_verification_status
  ...
  failures = mcps.select { |_, data| data.is_a?(Hash) && data[:failure_count].to_i.positive? }
...
begin
  ...
  log_debug 'show_mcp_verification_status done - SUCCESS'
rescue StandardError => e
