...
def track_requirement_satisfaction(tool_name, tool_input)
  ...
  requested.each do |req|
    ...
    next unless config[:satisfied_by].is_a?(Array) && config[:satisfied_by].first != :all_research_complete

    mark_requirement_satisfied(req) if config[:satisfied_by].any? { |p| input_text.match?(p) }
  end
end
